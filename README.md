...
完整版 16 个智能体，分为接入、编排、执行、监督四层：

| 层 | 智能体 |
|---|---|
| 接入 | 会话网关 |
| 编排 | 主控调度 Supervisor · 记忆管家 |
...
| 监控与交互的关系 | 同一个执行内核 + 两个触发适配器 | 避免两套口径；监控产出经记忆沉淀成为交互的输入 |
| 监控成本控制 | 三级执行深度（L0 全量扫描 / L1 研判 / L2 处置） | 全量走完整链路成本不可控；绝大多数扫描应以静默收场 |
| 执行图的生成方式 | 能力模板为主 + 模型兜底 + **契约校验器把关** | 让模型自由选执行者会幻觉、不可验证；校验器是可验证性的最后闸门 |
| 记忆读取时序 | 任务级**预取一次**，各步从结果中窄筛选 | 各智能体独立检索会造成重复计算与判断依据不一致 |
| 长期记忆写入时机 | 执行中只写短期记忆，**任务末尾统一蒸馏** | 每步都写会造成记忆爆炸与假冲突；写入路径唯一才能保证来源可追溯 |
| 任务内记忆版本 | **固定快照**，不中途换版本（MVCC 语义） | 否则前后半段判断依据不一致，结论无法审计 |
...
| 文档 | 内容 |
|---|---|
| [01 整体架构与智能体设计](docs/01_整体架构与智能体设计.md) | 六层架构、Agent Card 规范、16 个智能体职责、混合编排模型、任务 DAG 示例 |
...
| [08 监控与交互的统一设计](docs/08_监控与交互的统一设计.md) | 主动监控与按需交互如何统一：统一任务抽象、三级执行深度、记忆闭环、事件源与监测模板、告警治理、记忆沉淀策略 |
| [09 执行图的动态生成机制](docs/09_执行图的动态生成机制.md) | 调度器如何按任务生成执行图：意图识别、能力需求推导、能力索引匹配、依赖推导、契约校验器、运行期局部重规划 |
| [10 记忆流的检索与写入时序](docs/10_记忆流的检索与写入时序.md) | 记忆何时读何时写：预取与精准两级检索、执行中只写短期记忆、任务末尾统一蒸馏、使用标记回填、任务内快照语义 |
| [11 MVP 分步实施计划](docs/11_MVP分步实施计划.md) | 编码阶段的逐步任务分解：M0~M6 各自的目标、交付物、验收标准、允许简化项与禁止项 |

---

## 七、编码实施入口

**准备开始编码时，请先读根目录的 [`CODEBUDDY.md`](CODEBUDDY.md)。**

它是项目的编码实施指南（AI 编码代理会自动加载），包含：第一原则「内容可以假，机制必须真」、三条防做假铁律、必读文档地图、目录约定、MVP 路线图、禁止事项与汇报规范。逐步的任务分解与验收标准在 [docs/11](docs/11_MVP分步实施计划.md)。

MVP 按六个里程碑串行推进，每个里程碑验收通过才进入下一个：

| 里程碑 | 目标 |
|---|---|
| M0 | 冻结协议层，能启动并加载注册表 |
| M1 | 1 主控 + 1 执行智能体 + 3 技能 + 1 条记忆，端到端跑通 |
| M2 | 8 智能体 + 能力模板 + 依赖推导 + 契约校验器 |
| M3 | 三层记忆与六级时序 |
| M4 | 评审、熔断、仲裁、降级、局部重规划 |
| M5 | 技能治理（权限、限流、缓存、健康度） |
| M6 | 自检全绿 + trace 回放 + 事件流 |
